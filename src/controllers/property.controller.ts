...
export const addProperty = async (req: Request, res: Response) => {
    try {
        ...
        propertyData.statusInfo.userId = req.user.user._id;
        ...
        Property.find({ $and: [
          ...
          { 'statusInfo.status': propertyData.statusInfo.status },
            ...
            if (error) { return failureResponse(res, error.status || 500, error, error.message || 'Something went wrong'); }
              ...
              const userProperty = propertyExist.filter((x) => x.statusInfo.userId == propertyData.statusInfo.userId);
              if (userProperty && userProperty.length) {
                return failureResponse(res, 403, [], 'Property already exist with this value');
              }
              else {
                propertyData.statusInfo.version = propertyExist[propertyExist.length - 1].statusInfo.version + 1;
              }
            }
