...
export const addProperty = async (req: Request, res: Response) => {
    try {
        ...
        propertyData.propertyDetail.userId = new Types.ObjectId(req.user.user._id);
        ...
        Property.find({ $and: [
          ...
          { status: propertyData.status },
            ...
            if (error) { return failureResponse(res, error.status || 500, error, error.message || 'Something went wrong'); }
              ...
              const userProperty = propertyExist[0].propertyDetails.filter((x) => x.userId.equals(propertyData.propertyDetail.userId));
              if (userProperty && userProperty.length) {
                return failureResponse(res, 403, [], 'Property already exist with this value');
              }
              else {
                propertyData.propertyDetail.version = propertyExist[0].propertyDetails.length + 1;
                const detailObj =  new PropertyDetail(propertyData.propertyDetail);
