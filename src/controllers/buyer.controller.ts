    ...
    try {
        ...
        .exec(async (error: any, buyerExist: any) => {
          if (error) { return failureResponse(res, error.status || 500, error, error.message || 'Something went wrong'); }
            ...
            const buyerObj = buyerExist[0].buyerDetails.filter((x) => x.userId.equals(tempData.buyerData.userId));
...
const updateBuyerDetails = (id, detailsId, res) => {
  ...
  Buyer.findByIdAndUpdate({ _id: id }, { $push: { buyerDetails: detailId } }, { new: true })
    .populate('buyerDetails').exec((error, updatedRecord) => {
        if (error) {
            console.log('error while update', error);
            return failureResponse(res, 500, [], error.message || 'Something went wrong');
        } else {
            console.log('updatedRecord.......', updatedRecord);
            return successResponse(res, 200, { buyer: updatedRecord }, 'New buyer added successfully.');
         }
    });
};
